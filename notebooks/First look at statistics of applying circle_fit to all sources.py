...
for group, color in groups_and_colors:
    ...
    ax.scatter(x=dff_merge[m]["Pi"], y=dff_merge[m]["Lambda"], 
               marker="o", c=[color], s=sizes[m], label=group, edgecolors="w", zorder=3)
ax.legend(ncol=2).set_title("Group")    
ax.set(xlim=[0.35, 15.0], ylim=[0.35, 15.0], xscale="log", yscale="log")
ax.set_aspect("equal")
...
ax.set(
    xscale="log", yscale="log", 
    xlim=[0.75, 15], ylim=[0.75, 15],
    xlabel=r"Planitude, $\Pi$", ylabel=r"Alatude, $\Lambda$",
)
ax.set_aspect("equal");
...
bgmm = BayesianGaussianMixture(n_components=30, verbose=1, weight_concentration_prior=1000.0).fit(X)
...
rel_err = np.hypot(dff_merge[m]["e_Pi"]/dff_merge[m]["Pi"], dff_merge[m]["e_Lambda"]/dff_merge[m]["Lambda"])
...
GAMMA = 0.05
ax.contourf(xx, yy, p_tot**GAMMA, levels=levels**GAMMA, cmap="Greens", zorder=-1, alpha=0.5)
ax.scatter(
    10**X[:, 0], 10**X[:, 1], 
    c=plabels, s=5*np.sqrt(weights), 
