...
#+BEGIN_SRC python :return figlist :results list
  ...
  D60 = tab['D']/60
  contrast = tab['Dif_Bally']/tab['Value_bg_Bally']
  q = tab['R_out']/tab['D']
  ...
  cb = plt.colorbar()
  cb.set_label('Distance from th1C, D / arcmin')
  plt.xlabel('Bowshock fractional size, q = r0/D')
  plt.ylabel('Bowshock openness, A = Rc/r0')
  ax.set_xscale('log')
  fig.savefig(pltfile)
  ...
  fig = plt.figure(figsize=(7,6))
  ax = fig.add_subplot(111, axisbg="#eeeeee")
  plt.scatter(A[m], H[m], s=20*tab['R_out'][m], c=contrast[m], vmin=0.0, vmax=0.2, cmap=plt.cm.hot, alpha=0.6)
  # plt.scatter(0.5*(Ain[m] + A[m]), H[m], c=tab['D'][m]/60, vmin=0.0, cmap=plt.cm.hot_r)
  cb = plt.colorbar()
  cb.set_label('Shell/background brightness contrast')
  plt.xlabel('Bowshock openness, A = Rc/r0')
