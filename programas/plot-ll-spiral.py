...
def plot_map(
    ...
    fig = aplpy.FITSFigure(expand_fits_path(fitsfile), figsize=canvas_size, north=north)
    # Set the viewport
    xc, yc = (limits[0] + limits[1]) / 2, (limits[2] + limits[3]) / 2
    w, h = limits[1] - limits[0], limits[3] - limits[2]
    fig.recenter(
        c0.ra.deg - xc / 3600, c0.dec.deg + yc / 3600, width=w / 3600, height=h / 3600
    )
    fig.show_grayscale(
        vmin=vmin, vmax=vmax, invert=True, stretch=stretch, interpolation="none"
    )
    ax = plt.gca()
...
if __name__ == "__main__":
    ...
    # Set up arc data
    #
    table = Table.read("luis-programas/arcs-summary-classify.ecsv", format="ascii.ecsv")
    ...
    c0 = coord.SkyCoord("05:35:16.463", "-05:23:23.18", unit=(u.hourangle, u.degree))

    fullbox = [-425 - 200, 875 + 200, -825 - 200, 375 + 200]
    plot_map(
        fullbox, "ll-pos-spiral.pdf", (10, 10), vmin=1.0, vmax=2000.0, stretch="log"
    )
